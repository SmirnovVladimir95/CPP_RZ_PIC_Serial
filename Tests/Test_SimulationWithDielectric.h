#ifndef CPP_RZ_PIC_TEST_SIMULATIONWITHDIELECTRIC_H
#define CPP_RZ_PIC_TEST_SIMULATIONWITHDIELECTRIC_H
...
#include "../InteractionWithMaterial/Dielectric.h"
#include <numeric>
#include <algorithm>
...
void test_SimulationWithDielectric() {
    cout << "test_SimulationWithDielectric: " << endl;
    ...
    // Particle leave
    Matrix domain_condition_leave(Nz, Nr);
    for (int row = 0; row < domain_condition_leave.rows(); row++) {
        for (int col = 0; col < domain_condition_leave.columns(); col++) {
            if ((row == 0 or row == Nz - 1) or (col == Nr - 1))
                domain_condition_leave(row, col) = 1;
        }
    }
    ParticleLeave electrons_leave(electrons, grid, domain_condition_leave);
    ParticleLeave ions_leave(ions, grid, domain_condition_leave);
    ...
    scalar gamma = 0.3, emission_energy = 10*EV;
    ...
    // Electron/ion dielectric
    Matrix domain_condition_dielectric(Nz, Nr);
    for (int row = 0; row < domain_condition_dielectric.rows(); row++) {
        for (int col = 0; col < domain_condition_dielectric.columns(); col++) {
            if (row == 1 and col >= Nr / 2 and col < Nr - 1) {
                domain_condition_dielectric(row, col) = 1;
            } else if (row == Nz - 2 and col >= Nr / 2 and col < Nr - 1) {
                domain_condition_dielectric(row, col) = 3;
            }
        }
    }
    scalar dielectric_const = 3;
    Dielectric dielectric_electron(electrons, grid, domain_condition_dielectric, dielectric_const);
    Dielectric dielectric_ion(ions, grid, domain_condition_dielectric, dielectric_const);

    Matrix domain(Nz, Nr);
    domain = domain_condition_leave + domain_condition_left + domain_condition_right + domain_condition_dielectric;
    ...
    for (int it = 0; it < it_num; it++) {
        ...
        rho = electrons.rho + ions.rho + dielectric_electron.rho + dielectric_ion.rho;
        ...
        dielectric_electron.update_rho();
        if (it % ion_step == 0) {
            ions_leave.leave();
            emission_left.emission();
            emission_right.emission();
            dielectric_ion.update_rho();
        }
        /*
...
#endif //CPP_RZ_PIC_TEST_SIMULATIONWITHDIELECTRIC_H
